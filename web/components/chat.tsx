...
class Chat extends React.Component<Props, State> {
  ...
  componentWillUnmount() {
    if (this.props.isDev) location.reload();
  }
    ...
    client.onmessage = ({ data: wsData }) => {
      ...
      if (topics == "chatters") {
        ...
        msg.message = `Chatters:${[
          msg.map((chatter) => {
            return ` <span style="font-weight: 400" data-name="${
              chatter.username
            }">${chatter.username.replace(/^\w/, (c) => {
  ...
  private sendMsg(e: React.KeyboardEvent<HTMLInputElement>): void {
    ...
    if (colons % 2 == 0)
      this.setState({ emoteSearching: false, emoteMenuOpen: false });
  ...
  private emoteSearch(): HTMLElement {
    ...
    return emoteSearched.map((emote: any) => (
      <img
      ...
      >
        ...
        {emoteMenuOpen && (
          <>
            ...
            {emoteSearching && (
              <div className="emoteProviders search">
                Searching for <span>{emoteSearch}</span>
              </div>
            )}
            {!emoteSearching && (
              <div className="emoteProviders">
                ...
                >
                  <img
                    ...
                    src="https://timcole.me/static/logo.png"
                  />
                </div>
                <div
                  className={`emoteProvider${
                ...
                >
                  <img
                    className="emoteProviderLogo"
                    src="https://frankerfacez.com/static/images/favicon-32.png"
                  />
                </div>
                <div
                  className={`emoteProvider${
                ...
                >
                  <img
                    className="emoteProviderLogo"
                    src="https://www.twitch.tv/favicon.ico"
                  />
                </div>
              </div>
            )}
