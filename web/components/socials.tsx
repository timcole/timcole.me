...
class Stats extends React.Component<any, any> {
  ...
  async getStats(): Promise<void> {
    this.setState({
      discord: (
        await fetch(
          "https://discordapp.com/api/v6/invite/YFtfGwq?with_counts=true"
        ).then((data) => data.json())
      ).approximate_member_count.toLocaleString(),
      github: (
        await fetch("https://api.github.com/users/timothycole").then((data) =>
          data.json()
        )
      ).followers.toLocaleString(),
      notify: (
        await fetch("https://timcole.me/api/notify").then((data) => data.json())
      ).channel.subCount.toLocaleString(),
    });

    const socialblade = (
      await fetch("https://timcole.me/api/stats").then((data) => data.json())
    ).results;
    socialblade.map((result: any): void => {
      if (
        result.username != "modesttim" &&
        result.channelid != "UCLvM67Ey6kNCasyEV2dvrbA"
      )
        return;
      this.setState({
        [result.type.toLowerCase()]: Number(
          `${result.followers || result.subscribers}`.replace(/,/g, "")
        ).toLocaleString(),
      });
    });
  }
  ...
  private socials: string[][] = [
    ...
    [
      ...
      "https://www.youtube.com/user/EatTim?sub_confirmation=1",
      "youtube",
    ],
      ...
      <div className={this.props.className}>
        {this.socials.map((v: any) => (
          <a href={v[ESocial.URL]} target="_blank" key={v[ESocial.NETWORK]}>
            <div
              className={`button ${v[ESocial.NETWORK]}`}
              data-tooltip={`${v[ESocial.TOOLTIP]} - ${
                this.state[v[ESocial.NETWORK]] || "loading"
              }`}
            >
