    ...
    return (
      <div className="modal-mask">
        <div className="modal-wrapper" onClick={this.closeModal.bind(this)}>
          <Query query={subQuery} variables={{ login: "modesttim" }}>
            {({ loading, error, data }: any) => {
              if (loading) return <p>Loading...</p>;
              if (error) return <p>Error :(</p>;
              const product =
                data.user.subscriptionProducts[this.state.selected];
              return (
                <div className="modal-container">
                  <div className="modal-header">
                    <h2>{product.displayName}</h2>
                    <div onClick={() => this.popout(product.url)}>
                      Continue to Twitch
                    </div>
                  </div>
                  ...
                  <div className="modal-body">
                    <div className="m-left">
                      ...
                      <ul>
                        ...
                        {product.hasAdFree ? (
                          <li>Ad-free (with limited exceptions)</li>
                        ) : null}
                    ...
                    <div className="m-right">
                      ...
                      <div className="flex-parent">
                        {data.user.broadcastBadges.map((badge: any) => {
                          ...
                          const style = {
                            order: this.badges.indexOf(badge.description),
                          };
                          return (
                            <div
                              key={badge.imageURL}
                            ...
                            >
                              <img
                                src={badge.imageURL.replace("/1", "/3")}
                      ...
                      <h5>
                        {this.emotes(data.user.subscriptionProducts).length}{" "}
                        Subscriber Emote:
                      </h5>
                      <div className="flex-parent">
                        {this.emotes(data.user.subscriptionProducts).map(
                          (emote: any) => (
                            <div
                              key={emote.id}
                              className="ToolTipContainer"
                              data-tooltip={emote.token}
                            >
                              <img
                                src={`https://static-cdn.jtvnw.net/emoticons/v1/${emote.id}/4.0`}
                                alt={emote.token}
                              />
                            </div>
                          )
                  ...
                  <div className="modal-footer">
                    {data.user.subscriptionProducts.map(
                      (tier: any, index: number) => (
                        <div
                          ...
                          onClick={(a: any) =>
                            this.setState({
                              selected: Number(a.currentTarget.dataset.tier),
                            })
                          }
                        >
                          <p>
                            Tier {index + 1}{" "}
                            <span>
                              ${(tier.priceInfo.total / 100).toFixed(2)}
                            </span>
                          </p>
                        </div>
                      )
