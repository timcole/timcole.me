  ...
  return (launch && time)
    ? (
      <div class="items-center relative p-2 -mx-3 -mt-3 flex rounded-lg text-gray-800 dark:text-gray-100 shadow-lg bg-gray-100 dark:bg-gray-900 overflow-hidden">
        <div class="z-10 flex flex-col md:flex-row w-full items-center md:text-left">
          <p class="flex-1 pl-4">
            There{launch?.status === "Go" ? "'s going to" : " might"}{" "}
        ...
        <div
          style={`background-image: url('https://constellation.spaceflight.live/${launch?.vehicle.image}')`}
          class="left-0 opacity-30 absolute select-none h-full w-full bg-center bg-cover"
        >
        </div>
      </div>
    )
    : /* This is only here to prevent a layout shift when the data loads lol */ <div class="h-[74px] -mt-3">
    </div>;
};
